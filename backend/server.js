...
app.get('/api/events', async (req, res) => {
  try {
    ...
    let query = `SELECT e.*,
      (SELECT ISNULL(SUM(quantity_available), 0) FROM ticket_types WHERE event_id = e.event_id) as total_tickets,
      (SELECT ISNULL(SUM(quantity_sold), 0) FROM ticket_types WHERE event_id = e.event_id) as total_sold
      FROM events e`;
    ...
    if (ticketTypes && ticketTypes.length > 0) {
      for (const ticket of ticketTypes) {
        await pool.request()
          ...
          .query('INSERT INTO ticket_types (event_id, name, price, quantity_available, quantity_sold) VALUES (@eventId, @name, @price, @quantity, 0)');
      }
    }
...
app.put('/api/events/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (ticketTypes && Array.isArray(ticketTypes)) {
      // Check if any tickets have been sold for this event's ticket types
      const soldCheck = await pool.request()
        .input('eventId2', sql.Int, eventId)
        .query('SELECT ISNULL(SUM(quantity_sold), 0) as total_sold FROM ticket_types WHERE event_id = @eventId2');
      const totalSold = soldCheck.recordset[0].total_sold;

      if (totalSold > 0) {
        // Don't delete ticket types that have sales — update existing ones and add new ones
        // For simplicity, only allow full replacement if no tickets have been sold
        return res.status(400).json({ error: 'Không thể thay đổi loại vé khi đã có vé được bán. Vui lòng liên hệ quản trị viên.' });
      }

      // Delete old ticket types and insert new ones
      await pool.request()
        .input('eventId3', sql.Int, eventId)
        .query('DELETE FROM ticket_types WHERE event_id = @eventId3');

      for (const ticket of ticketTypes) {
        if (ticket.name && ticket.name.trim()) {
          await pool.request()
            .input('eventId4', sql.Int, eventId)
            .input('ttName', sql.NVarChar, ticket.name)
            .input('ttPrice', sql.Int, ticket.price || 0)
            .input('ttQty', sql.Int, ticket.quantity || 0)
            .query('INSERT INTO ticket_types (event_id, name, price, quantity_available, quantity_sold) VALUES (@eventId4, @ttName, @ttPrice, @ttQty, 0)');
        }
      }
    }
...
app.post('/api/orders', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const eventCheck = await new sql.Request(transaction)
      ...
      .query('SELECT event_id FROM events WHERE event_id = @eventId');
    ...
    if (!resolvedTicketTypeId && ticket_type_name) {
      const ttResult = await new sql.Request(transaction)
        .input('eventId', sql.Int, event_id)
        .input('ttName', sql.NVarChar, ticket_type_name)
        .query('SELECT ticket_type_id FROM ticket_types WHERE event_id = @eventId AND name = @ttName');
      if (ttResult.recordset.length > 0) {
        resolvedTicketTypeId = ttResult.recordset[0].ticket_type_id;
      }
    }

    // Lock the ticket type row and check availability
    if (resolvedTicketTypeId) {
      const stockCheck = await new sql.Request(transaction)
        .input('ttId', sql.Int, resolvedTicketTypeId)
        .query('SELECT ticket_type_id, quantity_available, quantity_sold FROM ticket_types WITH (UPDLOCK, HOLDLOCK) WHERE ticket_type_id = @ttId');

      if (stockCheck.recordset.length > 0) {
        const tt = stockCheck.recordset[0];
        const available = tt.quantity_available - tt.quantity_sold;
        if (available < quantity) {
          await transaction.rollback();
          return res.status(409).json({
            error: `Không đủ vé. Chỉ còn ${available} vé có sẵn.`,
            available
          });
        }
      }
    ...
    // Create tickets for each quantity(loop)
    ...
    if (resolvedTicketTypeId) {
      await new sql.Request(transaction)
        ...
        .query('UPDATE ticket_types SET quantity_sold = quantity_sold + @qty WHERE ticket_type_id = @ttId');
    }
    ...
    try { await transaction.rollback(); } catch (rollbackErr) { /* already rolled back */ }
    console.error('Create order error:', error);
    res.status(500).json({ error: 'Đặt vé thất bại. Vui lòng thử lại.' });
  }
});
...
// Routes - Tickets Stats (for dashboard summary)
app.get('/api/tickets/stats', authenticateToken, async (req, res) => {
  try {
    console.log('[TICKETS] Stats request received, user:', req.user);
    
    // Verify admin access
    const userQuery = 'SELECT role FROM users WHERE user_id = @userId';
    const userResult = await pool.request().input('userId', sql.Int, req.user.user_id).query(userQuery);

    console.log('[TICKETS] User query result:', userResult.recordset);

    if (userResult.recordset.length === 0 || userResult.recordset[0].role !== 'admin') {
      console.log('[TICKETS] Admin access denied');
      return res.status(403).json({ error: 'Admin access required' });
    }

    // Query 1: Total tickets sold, total revenue, total customers
    const statsQuery = `
      SELECT
        (SELECT ISNULL(SUM(quantity), 0) FROM orders WHERE status = 'completed') as totalTickets,
        (SELECT ISNULL(SUM(total_price), 0) FROM orders WHERE status = 'completed') as totalRevenue,
        (SELECT COUNT(DISTINCT user_id) FROM orders WHERE status = 'completed') as totalCustomers
    `;
    
    const statsResult = await pool.request().query(statsQuery);
    const stats = statsResult.recordset[0];
    console.log('[TICKETS] Stats query result:', stats);

    // Query 2: Top ticket type (most sold)
    const topTicketQuery = `
      SELECT TOP 1 
        tt.name as name,
        SUM(o.quantity) as totalSold
      FROM orders o
      JOIN ticket_types tt ON o.ticket_type_id = tt.ticket_type_id
      WHERE o.status = 'completed'
      GROUP BY tt.name
      ORDER BY SUM(o.quantity) DESC
    `;
    
    const topTicketResult = await pool.request().query(topTicketQuery);
    const topTicket = topTicketResult.recordset[0] || { name: 'N/A', totalSold: 0 };
    console.log('[TICKETS] Top ticket query result:', topTicket);
    ...
    const topBuyerQuery = `
      SELECT TOP 1 
        u.user_id,
        u.name as buyerName,
        SUM(o.quantity) as ticketsBought
      FROM orders o
    ...
    const topBuyerResult = await pool.request().query(topBuyerQuery);
    const topBuyer = topBuyerResult.recordset[0] || { buyerName: 'N/A', ticketsBought: 0 };
    console.log('[TICKETS] Top buyer query result:', topBuyer);
    ...
    const responseData = {
      stats: {
        ...
        topTicket: topTicket.name,
        topTicketSold: topTicket.totalSold,
        topBuyer: topBuyer.buyerName,
        topBuyerCount: topBuyer.ticketsBought
      }
    };
    
    console.log('[TICKETS] Sending response:', responseData);
    res.json(responseData);
  } catch (error) {
    console.error('[TICKETS] Stats error:', error);
    res.status(500).json({ error: 'Failed to fetch tickets stats', details: error.message });
  }
});
