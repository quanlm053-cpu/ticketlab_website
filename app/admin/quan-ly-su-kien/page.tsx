...
interface ApiEvent {
  ...
  price: number;
  image: string;
  ticketTypes?: string | Array<{ name: string; price: number; quantity: number; quantity_available?: number; quantity_sold?: number }>;
  total_tickets?: number;
  total_sold?: number;
}

const defaultTicketType: TicketTypeForm = { name: "", price: 0, quantity: 0 };
...
const defaultForm = {
  ...
  price: 0,
  image: "",
  ticketTypes: [{ ...defaultTicketType }] as TicketTypeForm[],
};
...
export default function AdminEventManagement() {
  ...
  const handleViewEvent = async (event: ApiEvent) => {
    try {
      const data = await eventsApi.getById(event.event_id);
  ...
  const handleEditEvent = async (event: ApiEvent) => {
    setSelectedEvent(event);
    // Fetch full event detail to get ticket types
    let ticketTypes: TicketTypeForm[] = [{ ...defaultTicketType }];
    try {
      const data = await eventsApi.getById(event.event_id);
      const fullEvent = data.event;
      const parsed = parseTicketTypes(fullEvent);
      if (parsed.length > 0) {
        ticketTypes = parsed.map((tt: any) => ({
          name: tt.name || "",
          price: tt.price || 0,
          quantity: tt.quantity_available ?? tt.quantity ?? 0,
        }));
      }
    } catch (err) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await eventsApi.update(selectedEvent.event_id, { ...editForm, image: imagePath, ticketTypes: editForm.ticketTypes });
  ...
  const handleAddEvent = async () => {
    try {
      ...
      await eventsApi.create({ ...addForm, image: imagePath, ticketTypes: addForm.ticketTypes });
                  ...
                  {event.total_tickets != null && (
                    <div className="mt-2">
                      <div className="flex items-center justify-between text-xs text-[#666666]">
                        <span>Còn lại: {(event.total_tickets - (event.total_sold || 0))}/{event.total_tickets} vé</span>
                        <span className="font-semibold text-[#2d5f5d]">{event.total_tickets > 0 ? Math.round(((event.total_sold || 0) / event.total_tickets) * 100) : 0}% đã bán</span>
                      </div>
                      <div className="mt-1 h-1.5 w-full rounded-full bg-[#e0e0e0]">
                        <div
                          className="h-full rounded-full bg-[#2d5f5d]"
                          style={{ width: `${event.total_tickets > 0 ? ((event.total_sold || 0) / event.total_tickets) * 100 : 0}%` }}
                        />
                      </div>
                    </div>
                  )}
          ...
          <div className="max-h-[90vh] max-w-2xl overflow-y-auto rounded-lg bg-[#ffffff]">
            ...
            <div className="p-6">
              ...
              {parseTicketTypes(selectedEvent).length > 0 && (() => {
                const tickets = parseTicketTypes(selectedEvent);
                const totalAvailable = tickets.reduce((sum: number, tt: any) => sum + (tt.quantity_available ?? tt.quantity ?? 0), 0);
                const totalSold = tickets.reduce((sum: number, tt: any) => sum + (tt.quantity_sold ?? 0), 0);
                return (
                  <div className="mt-6 border-t border-[#e0e0e0] pt-6">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-[#1a1a1a]">Loại vé</h3>
                      <span className="text-sm font-semibold text-[#2d5f5d]">
                        Còn lại: {totalAvailable - totalSold}/{totalAvailable} vé
                      </span>
                    </div>
                    <div className="mt-3 space-y-3">
                      {tickets.map((tt: any, i: number) => {
                        const available = tt.quantity_available ?? tt.quantity ?? 0;
                        const sold = tt.quantity_sold ?? 0;
                        const remaining = available - sold;
                        const percent = available > 0 ? (sold / available) * 100 : 0;
                        return (
                          <div key={i} className="rounded-lg border border-[#e0e0e0] px-4 py-3">
                            <div className="flex items-center justify-between text-sm">
                              <span className="font-semibold text-[#1a1a1a]">{tt.name}</span>
                              <span className="font-semibold text-[#2d5f5d]">{(tt.price || 0).toLocaleString("vi-VN")} đ</span>
                            </div>
                            <div className="mt-2 flex items-center justify-between text-xs text-[#666666]">
                              <span>Đã bán: {sold}/{available}</span>
                              <span>Còn lại: {remaining}</span>
                            </div>
                            <div className="mt-1.5 h-2 w-full rounded-full bg-[#e0e0e0]">
                              <div
                                className={`h-full rounded-full ${percent >= 90 ? 'bg-red-500' : percent >= 70 ? 'bg-[#c8a96e]' : 'bg-[#2d5f5d]'}`}
                                style={{ width: `${percent}%` }}
                              />
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#000000]/50 p-4">
          <div className="max-h-[90vh] max-w-2xl overflow-y-auto rounded-lg bg-[#ffffff] w-full">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="block text-sm font-semibold text-[#1a1a1a]">Loại vé</label>
                  <button
                    type="button"
                    onClick={() => setEditForm({ ...editForm, ticketTypes: [...editForm.ticketTypes, { ...defaultTicketType }] })}
                    className="flex items-center gap-1 text-sm font-semibold text-[#2d5f5d] hover:text-[#245250]"
                  >
                ...
                <div className="mt-2 space-y-2">
                  {editForm.ticketTypes.map((tt, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <input
                        ...
                        value={tt.name}
                        onChange={(e) => {
                          const updated = [...editForm.ticketTypes];
                          updated[idx] = { ...updated[idx], name: e.target.value };
                          setEditForm({ ...editForm, ticketTypes: updated });
                        }}
                        className="flex-1 rounded border border-[#e0e0e0] px-3 py-2 text-sm text-[#1a1a1a] focus:border-[#2d5f5d] focus:outline-none"
                      />
                      <input
                        type="number"
                        placeholder="Giá"
                        ...
                        className="w-28 rounded border border-[#e0e0e0] px-3 py-2 text-sm text-[#1a1a1a] focus:border-[#2d5f5d] focus:outline-none"
                      />
                      <input
                        type="number"
                        ...
                        className="w-24 rounded border border-[#e0e0e0] px-3 py-2 text-sm text-[#1a1a1a] focus:border-[#2d5f5d] focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={() => {
                          const updated = editForm.ticketTypes.filter((_, i) => i !== idx);
                          setEditForm({ ...editForm, ticketTypes: updated });
                        }}
                        disabled={editForm.ticketTypes.length <= 1}
                        className="rounded p-2 text-red-600 hover:bg-red-50 disabled:opacity-30 disabled:cursor-not-allowed"
                      >
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setAddForm({ ...addForm, ticketTypes: [...addForm.ticketTypes, { ...defaultTicketType }] })}
                    className="flex items-center gap-1 text-sm font-semibold text-[#2d5f5d] hover:text-[#245250]"
                  >
                    <Plus className="h-3 w-3" />
                    Thêm loại vé
                ...
                <div className="mt-2 space-y-2">
                  {addForm.ticketTypes.map((tt, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <input
                        type="text"
                        placeholder="Tên loại vé"
                        value={tt.name}
                        onChange={(e) => {
                          const updated = [...addForm.ticketTypes];
                          updated[idx] = { ...updated[idx], name: e.target.value };
                          setAddForm({ ...addForm, ticketTypes: updated });
                        }}
                        className="flex-1 rounded border border-[#e0e0e0] px-3 py-2 text-sm text-[#1a1a1a] focus:border-[#2d5f5d] focus:outline-none"
                      />
                      <input
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const updated = addForm.ticketTypes.filter((_, i) => i !== idx);
                          setAddForm({ ...addForm, ticketTypes: updated });
                        }}
                        disabled={addForm.ticketTypes.length <= 1}
                        className="rounded p-2 text-red-600 hover:bg-red-50 disabled:opacity-30 disabled:cursor-not-allowed"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
